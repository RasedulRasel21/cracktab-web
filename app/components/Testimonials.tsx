...
/** Client testimonials, each with the headline results from that project. */
      ...
      <div className="mx-auto w-full max-w-360 px-5 sm:px-8">
        ...
        <ul className="mt-12 grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
          {testimonials.map((t) => (
            <li
              key={t.brand}
              className="rounded-2xl border border-line bg-surface p-7 transition-colors duration-300 hover:border-accent/50"
            >
              {/* figure/figcaption keeps the attribution tied to its quote */}
              <figure className="flex h-full flex-col">
                ...
                <ul className="mt-6 flex flex-wrap gap-2">
                  {t.metrics.map((m) => (
                    <li
                      key={m}
                      className="rounded-full border border-accent/30 bg-accent/10 px-3 py-1 text-[0.7rem] font-medium text-accent"
                    >
                      {m}
                    </li>
                  ))}
